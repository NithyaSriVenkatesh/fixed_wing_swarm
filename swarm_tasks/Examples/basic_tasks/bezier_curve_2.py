...
class BezierCurve():
            ...
            with open(
                ...
                for j in range(len(csv_datas[i])):
                    if number_of_waypoints < minimum_waypoints:
                        ...
                        y,x = geoToCart(self.origin,200000,csv_datas[i][j])
    ...
    def write_kml(self,data,num):
        ...
        for i,cmd in enumerate(data):
            lat,lon = cartToGeo(self.origin,200000,[cmd[0]*2,cmd[1]*2])
...
curve = BezierCurve(origin=origin,center_latitude=  13.372730,center_longitude=  80.238806,num_of_drones=5,grid_space=100,coverage_area=2000)
curve.GridFormation()
