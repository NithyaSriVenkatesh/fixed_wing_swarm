...
class AutoSplitMission():
    ...
    def __init__(self, origin,center_lat_lons, num_of_drones, grid_spacing, coverage_area):
        ...
        self.initial_heading = np.radians(0)
        ...
        self.waypoints = []
        self.sample_points = []
        self.curve_csv_file = "/home/casr-3/Documents/fixed_wing/swarm_tasks/Examples/basic_tasks/group_split/bezier/d{}.csv"
        self.search_csv_name = "/home/casr-3/Documents/fixed_wing/swarm_tasks/Examples/basic_tasks/group_split/bezier/search_{}.kml"
    ...
    def CreateGridsForSpecifiedAreaAndSpecifiedDrones(
        ...
        for i in range(num_rectangles):
            ...
            kml.save(
                "/home/casr-3/Documents/fixed_wing/swarm_tasks/Examples/basic_tasks/group_split/"
                + kml_filename
            )
    ...
    def GroupSplitting(
        ...
        print("len(center_lat_lons)",len(center_lat_lons))
        drones_array = [0] * len(center_lat_lons)
        print('drones_array',drones_array,num_of_drones)
        for i in range(num_of_drones):
            drones_array[i % len(center_lat_lons)] += 1
            print('drones_array111',drones_array)
        print('drones_array',drones_array)
        start = 1
        for i in range(len(center_lat_lons)):
            if drones_array[i] == 0:
                continue
            print('center_lat_lons',center_lat_lons[i])
    ...
    def generate_bezier_curve(self):
        for num in range(self.num_of_drones):
            print("num",num)
            waypoints = []
            csv_filename = f"search-drone-{num+1}.csv"
            with open(
                    "/home/casr-3/Documents/fixed_wing/swarm_tasks/Examples/basic_tasks/group_split/"
                    + csv_filename,
                    mode="r",
...
split = AutoSplitMission(origin,center_lat_lons=center_latlon, num_of_drones=num_of_drones, grid_spacing=grid_spacing,
